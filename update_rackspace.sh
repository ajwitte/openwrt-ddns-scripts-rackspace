...
local rs_identity_json=`curl -s --insecure https://identity.api.rackspacecloud.com/v2.0/tokens \
  -X POST \
...
local rs_domainid=`curl -s --insecure "$rs_endpoint/domains" \
...
local rs_recordid=`curl -s --insecure "$rs_endpoint/domains/$rs_domainid/records" \
  -H "X-Auth-Token: $rs_token" -H "Accept: application/json" \
  | jsonfilter -e "@.records[@.name=\"$domain\" && @.type=\"$rs_type\"].id"` || return 1

local rs_callback=`curl -s --insecure "$rs_endpoint/domains/$rs_domainid/records/$rs_recordid" \
  -X PUT -d "{\"data\":\"$__IP\"}" \
